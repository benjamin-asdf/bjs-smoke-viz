...
(defn advance
  ...
  (let [fl (f/vel-step fl (:visc p) (:dt p) (:buoy p))]
    (when (#{:smoke :trail} (mode p)) (phys/step! (:phys fl) fl p))
    (let [fl (-> fl
...
(defn- update-stars!
  "Drift existing stars by their small constant acceleration (they stay on
   screen), and spawn a new persistent star at each strong density peak that has
   no star nearby yet. Runs in `advance` so it works headless too."
  [fl p]
  (let [n (long N) wmax (double (dec W))
        ...
        moved (mapv (fn [s]
                      (let [vx (+ (double (:vx s)) (double (:ax s)))
                        ...
                        (assoc s :vx vx :vy vy
                               :x (min wmax (max 0.0 (+ (double (:x s)) vx)))
                               :y (min wmax (max 0.0 (+ (double (:y s)) vy))))))
                    @stars)
        acc (volatile! moved)]
    (dotimes [j n]
      (dotimes [i n]
